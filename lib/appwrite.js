...
export async function getCurrentUser() {
    try {
        ...
        // console.log('Current Account:', currentAccount, 'Current User:', currentUser.documents[0]);
...
//     console.log('appwrite.js -> getAllPosts -> posts -> ',posts)
...
export const uploadFile = async (file, type) => {
  console.log('appwrite.jsx -> uploadFile -> file', file);
  if (!file) return;
  ...
  console.log('appwrite.js -> uploadFile after re-naming -> file', file);
  try {
    const uploadedFile = await storage.createFile(
      appwriteConfig.storageId, ID.unique(), asset
    );

    console.log('appwrite.jsx -> uploadFile -> uploadedFile', uploadedFile);
...
export const createShops = async (form) => {
  try {
    console.log('appwrite.jsx -> createShops -> form', form);
